...
from matterport.model_util_matterport import MatterportDatasetConfig_md40, rotate_aligned_boxes
...
class MatterportSSLLabeledDataset(Dataset):

    def __init__(self, labeled_sample_list=None, num_points=20000, use_color=False, use_height=False, augment=False):

        print('--------- Matterport Labeled Dataset Initialization ---------')
        self.data_path = os.path.join(BASE_DIR, 'matterport_train_detection_data_md40')
        if labeled_sample_list is not None:
            self.scan_names = [x.strip() for x in open(
                os.path.join(ROOT_DIR, 'matterport/meta_data', labeled_sample_list)).readlines()]
...
class MatterportSSLUnlabeledDataset(Dataset):
    def __init__(self, labeled_sample_list=None, num_points=20000, use_color=False,
        ...
        print('----------------Matterport Unlabeled Dataset Initialization----------------')
        self.data_path = os.path.join(BASE_DIR, 'matterport_train_detection_data_md40')
        all_scan_names = list(set([os.path.basename(x)[0:12] \
                                   for x in os.listdir(self.data_path) if x.startswith('scene')]))
        split_filenames = os.path.join(ROOT_DIR, 'matterport/meta_data/matterport3d_train.txt')
