...
class MatterportDetectionDataset(Dataset):
    ...
    def __init__(self, split_set='train', labeled_ratio=0.1, labeled_sample_list=None, num_points=20000,
        ...
        # added
        self.raw_data_path = os.path.join(ROOT_DIR, 'matterport/meta_data')
    ...
    def get_labeled_samples(self):
        if self.labeled_sample_list is not None:
            labeled_scan_names = [x.strip() for x in open(
                os.path.join(ROOT_DIR, 'matterport/meta_data', self.labeled_sample_list)).readlines()]
        else:
            ...
            while True:
                ...
                if np.sum(class_mask) == DC.num_class:
                    ...
                    with open(os.path.join(ROOT_DIR, 'matterport/meta_data/matterport3d_train_{}.txt'.format(self.labeled_ratio)), 'w') as f:
