...
if FLAGS.dataset == 'matterport':
    ...
    from matterport.matterport_detection_dataset import MatterportDetectionDataset
    from matterport.matterport_ssl_dataset import MatterportSSLLabeledDataset, MatterportSSLUnlabeledDataset
    from matterport.model_util_matterport import MatterportDatasetConfig_md40
    DATASET_CONFIG = MatterportDatasetConfig_md40()
    LABELED_DATASET = MatterportSSLLabeledDataset(labeled_sample_list=FLAGS.labeled_sample_list,
                                                        num_points=NUM_POINT,
                                                        augment=True,
                                                        use_color=FLAGS.use_color,
                                                        use_height=(not FLAGS.no_height))
    UNLABELED_DATASET = MatterportSSLUnlabeledDataset(labeled_sample_list=FLAGS.labeled_sample_list,
                                                   num_points=NUM_POINT,
                                                   use_color=FLAGS.use_color,
                                                   use_height=(not FLAGS.no_height),
                                                   load_labels=FLAGS.view_stats,
                                                   augment=True)
    TEST_DATASET = MatterportDetectionDataset('val',
