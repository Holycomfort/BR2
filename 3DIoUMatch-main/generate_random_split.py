...
def gen_scannet_split(labeled_ratio, count):
    ...
    data_path = os.path.join(BASE_DIR, 'scannet/scannet_train_detection_data_md40')
    ...
    while True:
        ...
        if np.sum(class_mask) == DC.num_class:
            ...
            with open(os.path.join(ROOT_DIR, 'scannet/meta_data/scannetv2_train_{}.txt'.format(labeled_ratio)),
                      'w') as f:
