...
if __name__ == "__main__":
    ...
    for scan_name in scan_names:
        ...
        np.save('/home/xxw/3D/BR2/BackToReality/data_generation/ScanNet/augment_random_positions_scannet/' + scan_name + '.npy', positions)

    # augmentation
    sys.path.append(os.path.join(ROOT_DIR, 'detection' ,'votenet', 'scannet'))
    ...
    for d in dset:
        ...
        add_dresser = (scan_name in ls[2])
        add_laptop = (scan_name in ls[3])
        add_wardrobe = (scan_name in ls[4])
        positions, fw_points = generate_initial_random_positions(mesh_file, agg_file, seg_file, meta_file, modelnet40_path)
        ...
        for aug_idx in range(2):
            ...
            for key, value in positions.items():
                if value[2].split('/')[-2] in ['cup', 'bottle', 'keyboard'] and np.random.rand() > 0.25:
                    # object_id: [(x,y,z), (sx,sy,sz), object_txt, Is_supporter, theta, support_MER/None, height/None]
                    new_position = [(xmin+np.random.rand()*(xmax-xmin), ymin+np.random.rand()*(ymax-ymin), zmin+np.random.rand()*(zmax-zmin)), value[1], value[2], value[3], value[4], value[5], value[6]]
                    positions_aug[key_toadd] = new_position
                    key_toadd += 1
                    add_list.append(value[2].split('/')[-2])
            for scarce in ['bathtub', 'bench', 'dresser', 'laptop', 'wardrobe']:
                ...
                if scarce == 'dresser' and add_dresser == False:
                    continue
                if scarce == 'laptop' and add_laptop == False:
                    continue
                if scarce == 'wardrobe' and add_wardrobe == False:
                    continue
            ...
            np.save('/home/xxw/3D/BR2/BackToReality/data_generation/ScanNet/augment_random_positions_scannet/' + aug_scan_name + '.npy', positions_aug)
