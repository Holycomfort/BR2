...
def export(mesh_file, agg_file, seg_file, meta_file, output_file=None):
    ...
    adj_matrix, idx_to_segidx, segidx_to_idx = generate_seg_adjacency_matrix(plydata, segindices)
    ...
    # to simulate labeling of human
    instance_segidxs = []
    h_areas_dict = {}
    for object_id, segs in object_id_to_segs.items():
        h_areas = np.array([get_horizontal_area(mesh_vertices, seg_to_verts[segidx])
         for segidx in segs])
        is_h = np.array([Is_horizontal(mesh_vertices, seg_to_verts[segidx])
         for segidx in segs])
        h_areas *= is_h
        h_areas_dict[object_id] = h_areas
        max_area = np.max(h_areas)
        max_area_idx = np.argmax(h_areas)
        max_segidx = segs[max_area_idx]
        if max_area < 0.1:
            instance_segidxs.append(np.random.choice(segs))
        else:
            instance_segidxs.append(max_segidx)
    
    # obj_id: [Has_Plane, Plane/Random segidx, plane height, label, modelnet_id]
    hrz_obj_dict = {}
    ...
    obj_idx = -1
    for object_id, segs in object_id_to_segs.items():
        h_areas = h_areas_dict[object_id]
        max_area = np.max(h_areas)
        max_area_idx = np.argmax(h_areas)
        max_segidx = segs[max_area_idx]
        modelnet_id = label_map[id_to_label[object_id]]
        height = get_height(mesh_vertices, scene_normal, seg_to_verts[max_segidx])
        ...
        obj_idx += 1
        if max_area < 0.1 or np.isnan(height):
            ...
            if scan_name in scan_name_to_idx.keys():
                ...
                dx, dy, dz = 0, 0, 0
            x += dx; y += dy; z += dz
            hrz_obj_dict[object_id] = [False, (x,y,z), None, id_to_label[object_id]]
        else:
            neighbor_segidx = get_neighbor(adj_matrix, idx_to_segidx, segidx_to_idx, max_segidx)
            expanded_segidx = [max_segidx]
            explored_segidx = [max_segidx]
            # label_ids == 0 idicates this object is not in ModelNet40
            if label_ids[seg_to_verts[expanded_segidx[0]]][0] == 0:
                continue
            while len(neighbor_segidx) > 0:
                now_segidx = neighbor_segidx.pop()
                explored_segidx.append(now_segidx)
                if now_segidx in instance_segidxs:
                    continue
                now_height = get_height(mesh_vertices, scene_normal, seg_to_verts[now_segidx])
                if abs(height - now_height) < 0.02:
                    for x in get_neighbor(adj_matrix, idx_to_segidx, segidx_to_idx, now_segidx):
                        if x not in explored_segidx + neighbor_segidx:
                            neighbor_segidx.insert(0, x)
                    expanded_segidx.append(now_segidx)
            hrz_obj_dict[object_id] = [True, expanded_segidx, height, id_to_label[object_id]]
        if modelnet_id == 0:
            hrz_obj_dict.pop(object_id)
        else:
            hrz_obj_dict[object_id] += [modelnet_id]

    return hrz_obj_dict, seg_to_verts, mesh_vertices
...
def export_random(mesh_file, agg_file, seg_file, meta_file):
    ...
    for object_id, segs in object_id_to_segs.items():
        ...
        #if modelnet_id in [2,3,4,5,6,9,11,12,13,14,15,19,20,21,23,24,27,31,33,34,36,39]:
        #    obj_idx += 1
        if modelnet_id not in [2,3,4,5,6,9,11,12,13,14,15,19,20,21,23,24,27,31,33,34,36,39]: continue
        obj_idx += 1
        xmin = np.min(obj_pc[:,0])
