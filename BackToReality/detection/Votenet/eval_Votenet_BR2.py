...
LOG_FOUT = open(os.path.join(LOG_DIR, 'log_eval_%02d.txt'  % (FLAGS.center_jitter*100)), 'a')
...
CONFIG_DICT = {'dataset_config': DATASET_CONFIG, 'dataset': FLAGS.dataset,
               ...
               'obj_threshold': 0.9, 'cls_threshold': 0.9, 'use_lhs': True,
...
def evaluate_one_epoch():
    ...
    for batch_idx, batch_data_label_T in enumerate(TEST_DATALOADER_T):
        ...
        end_points_T['center'] = torch.cat([end_points_T['center'], ema_end_points_T['center']], dim=1)
        end_points_T['heading_scores'] = torch.cat([end_points_T['heading_scores'], ema_end_points_T['heading_scores']], dim=1)
        end_points_T['heading_residuals'] = torch.cat([end_points_T['heading_residuals'], ema_end_points_T['heading_residuals']], dim=1)
        end_points_T['size_scores'] = torch.cat([end_points_T['size_scores'], ema_end_points_T['size_scores']], dim=1)
        end_points_T['size_residuals'] = torch.cat([end_points_T['size_residuals'], ema_end_points_T['size_residuals']], dim=1)
        end_points_T['sem_cls_scores'] = torch.cat([end_points_T['sem_cls_scores'], ema_end_points_T['sem_cls_scores']], dim=1)
        end_points_T['objectness_scores'] = torch.cat([end_points_T['objectness_scores'], ema_end_points_T['objectness_scores']], dim=1)
