...
def evaluate_one_epoch():
    stat_dict = {} # collect statistics
    ap_calculator1 = APCalculator(ap_iou_thresh=0.25,
        class2type_map=DATASET_CONFIG.class2type)
    ap_calculator2 = APCalculator(ap_iou_thresh=0.5,
        class2type_map=DATASET_CONFIG.class2type)
    ...
    for batch_idx, batch_data_label in enumerate(TEST_DATALOADER):
        ...
        ap_calculator1.step(batch_pred_map_cls, batch_gt_map_cls)
        ap_calculator2.step(batch_pred_map_cls, batch_gt_map_cls)
    ...
    log_string('######## mAP@0.25 ########')
    metrics_dict = ap_calculator1.compute_metrics()
    for key in metrics_dict:
        log_string('eval %s: %f'%(key, metrics_dict[key]))
    log_string('######## mAP@0.5 ########')
    metrics_dict = ap_calculator2.compute_metrics()
    for key in metrics_dict:
        log_string('eval %s: %f'%(key, metrics_dict[key]))
