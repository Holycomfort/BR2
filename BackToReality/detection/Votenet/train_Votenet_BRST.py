...
parser.add_argument('--checkpoint_path_student', default=None, help='Model checkpoint path [default: None]')
parser.add_argument('--checkpoint_path_teacher', default=None, help='Model checkpoint path [default: None]')
...
CHECKPOINT_PATH_STUDENT = FLAGS.checkpoint_path_student if FLAGS.checkpoint_path_student is not None \
    else DEFAULT_CHECKPOINT_PATH
CHECKPOINT_PATH_TEACHER = FLAGS.checkpoint_path_teacher if FLAGS.checkpoint_path_teacher is not None \
    else DEFAULT_CHECKPOINT_PATH
...
def create_model(ema=False):
    model = Detector(num_class=DATASET_CONFIG.num_class,
...
ST_criterion = MODEL.get_loss_ST
weak_criterion = MODEL.get_loss_weak
...
start_epoch = 0
if CHECKPOINT_PATH_STUDENT is not None and os.path.isfile(CHECKPOINT_PATH_STUDENT):
    checkpoint = torch.load(CHECKPOINT_PATH_STUDENT)
    net.load_state_dict(checkpoint['model_state_dict'], strict=False)
    if FLAGS.resume:
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        start_epoch = checkpoint['epoch']
    log_string("-> loaded checkpoint %s (epoch: %d)"%(CHECKPOINT_PATH_STUDENT, start_epoch))
if CHECKPOINT_PATH_TEACHER is not None and os.path.isfile(CHECKPOINT_PATH_TEACHER):
    checkpoint = torch.load(CHECKPOINT_PATH_TEACHER)
    ema_net.load_state_dict(checkpoint['model_state_dict'], strict=False)
    log_string("-> loaded checkpoint %s"%(CHECKPOINT_PATH_TEACHER))
...
CONFIG_DICT = {'dataset_config': DATASET_CONFIG, 'dataset': FLAGS.dataset,
               'remove_empty_box': False, 'use_3d_nms': True, 'nms_iou': 0.25,
               'use_old_type_nms': False, 'cls_nms': True, 'conf_thresh': 0.05,
               'per_class_proposal': True, 'size_ratio': 0.3,
               'obj_threshold': 0.9, 'iou_threshold': 0.25, 'use_lhs': True,
               'vote_loss_size_factor': 1.0, 'samecls_match': False}
...
def train_one_epoch(global_step):
    ...
    for batch_idx, batch_data_label_T in enumerate(TRAIN_DATALOADER_T):
        ...
        with torch.no_grad():
            ema_end_points_T = ema_net(inputs_T)
        ...
        for key in batch_data_label_T:
            ...
            ema_end_points_T[key] = batch_data_label_T[key]

        #DA_loss, end_points_S, end_points_T = DA_criterion(end_points_S, end_points_T, DATASET_CONFIG)
        weak_loss, end_points_T = weak_criterion(end_points_T, DATASET_CONFIG)
        ST_loss, end_points_T, ema_end_points_T = ST_criterion(end_points_T, ema_end_points_T, DATASET_CONFIG, CONFIG_DICT)
        loss = ST_loss + weak_loss #+ DA_loss
        loss.backward()
        optimizer.step()
        global_step += 1
        #update_ema_variables(net, ema_net, FLAGS.ema_decay, global_step)
...
def evaluate_one_epoch():
    stat_dict = {} # collect statistics
    ap_calculator_T1 = APCalculator(ap_iou_thresh=0.25,
        class2type_map=DATASET_CONFIG.class2type)
    ap_calculator_T2 = APCalculator(ap_iou_thresh=0.5,
        class2type_map=DATASET_CONFIG.class2type)
    ema_ap_calculator_T1 = APCalculator(ap_iou_thresh=0.25,
        class2type_map=DATASET_CONFIG.class2type)
    ema_ap_calculator_T2 = APCalculator(ap_iou_thresh=0.5,
        class2type_map=DATASET_CONFIG.class2type)
    ...
    for batch_idx, batch_data_label_T in enumerate(TEST_DATALOADER_T):
        ...
        ap_calculator_T1.step(batch_pred_map_cls_T, batch_gt_map_cls_T)
        ap_calculator_T2.step(batch_pred_map_cls_T, batch_gt_map_cls_T)
        ema_batch_pred_map_cls_T = parse_predictions(ema_end_points_T, CONFIG_DICT) 
        ema_ap_calculator_T1.step(ema_batch_pred_map_cls_T, batch_gt_map_cls_T)
        ema_ap_calculator_T2.step(ema_batch_pred_map_cls_T, batch_gt_map_cls_T)


    # Evaluate average precision
    log_string("================= Target Student 0.25 =================")
    metrics_dict_T = ap_calculator_T1.compute_metrics()
    for key in metrics_dict_T:
        log_string('eval %s: %f'%(key, metrics_dict_T[key]))
    log_string("================= Target Student 0.5 =================")
    metrics_dict_T = ap_calculator_T2.compute_metrics()
    for key in metrics_dict_T:
        log_string('eval %s: %f'%(key, metrics_dict_T[key]))
    log_string("================= Target Teacher 0.25 =================")
    metrics_dict_T = ema_ap_calculator_T1.compute_metrics()
    for key in metrics_dict_T:
        log_string('eval %s: %f'%(key, metrics_dict_T[key]))
    log_string("================= Target Teacher 0.5 =================")
    metrics_dict_T = ema_ap_calculator_T2.compute_metrics()
    for key in metrics_dict_T:
        log_string('eval %s: %f'%(key, metrics_dict_T[key]))
