...
def guassian_kernel(source, target, sigma_list):
    ...
    for sigma in sigma_list:
        gamma = 1.0 / (2 * sigma**2)
        kernel_val.append(torch.exp(-gamma * L2_distance))
    return sum(kernel_val)


def mmd_rbf(source, target, sigma_list):
    batch_size = int(source.size()[0])
    kernels = guassian_kernel(source, target, sigma_list)
