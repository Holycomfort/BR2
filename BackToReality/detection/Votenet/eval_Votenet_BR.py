...
def evaluate_one_epoch():
    ...
    print("================= Target 0.25 =================")
    metrics_dict_T1 = ap_calculator_T1.compute_metrics()
    for key in metrics_dict_T1:
        log_string('eval %s: %f'%(key, metrics_dict_T1[key]))
    print("================= Target 0.5 =================")
    metrics_dict_T2 = ap_calculator_T2.compute_metrics()
    for key in metrics_dict_T2:
        log_string('eval %s: %f'%(key, metrics_dict_T2[key]))
        if 'mAP' in key:
            open(os.path.join(LOG_DIR, 'Eval_mAP.txt'), 'a').write('eval %s: %f%%\n'%(key, metrics_dict_T1[key] * 100))
