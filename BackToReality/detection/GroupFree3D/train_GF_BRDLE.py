...
from models import virtual_generator
from models import GroupFreeDetector_DA, get_loss_DA
...
def load_checkpoint(args, model, optimizer, scheduler, generator, virtual_optimizer, virtual_scheduler):
    ...
    virtual_optimizer.load_state_dict(checkpoint['virtual_optimizer'])
    virtual_scheduler.load_state_dict(checkpoint['virtual_scheduler'])
...
def save_checkpoint(args, epoch, model, optimizer, scheduler, generator, virtual_optimizer, virtual_scheduler, save_cur=False):
    logger.info('==> Saving...')
    state = {
        'config': args,
        'save_path': '',
        ...
        'virtual_optimizer': virtual_optimizer.state_dict(),
        'virtual_scheduler': virtual_scheduler.state_dict(),
        'epoch': epoch,
    }
...
def main(args):
    ...
    generator = virtual_generator(len(train_loader.dataset), num_points=args.num_point, dataset=args.dataset)
    if dist.get_rank() == 0:
    ...
    virtual_optimizer = optim.Adam(generator.parameters(), lr=args.virtual_learning_rate, eps=1e-4)
    virtual_scheduler = optim.lr_scheduler.MultiStepLR(virtual_optimizer, milestones=range(0, args.max_epoch, args.mstone), gamma=args.gamma)

    scheduler = get_scheduler(optimizer, len(train_loader), args)
    model = model.cuda()
    model = DistributedDataParallel(model, device_ids=[args.local_rank], broadcast_buffers=False)
    generator = generator.cuda()
    generator = DistributedDataParallel(generator, device_ids=[args.local_rank], broadcast_buffers=False)

    if args.checkpoint_path:
        assert os.path.isfile(args.checkpoint_path)
        load_checkpoint(args, model, optimizer, scheduler, generator, virtual_optimizer, virtual_scheduler)
    ...
    for epoch in range(args.start_epoch, args.max_epoch + 1):
        ...
        train_one_epoch(epoch, train_loader, DATASET_CONFIG, model, criterion, optimizer, scheduler, generator, virtual_optimizer, virtual_scheduler, args)
        ...
        if dist.get_rank() == 0:
            # save model
            save_checkpoint(args, epoch, model, optimizer, scheduler, generator, virtual_optimizer, virtual_scheduler)
    evaluate_one_epoch(test_loader, DATASET_CONFIG, CONFIG_DICT, args.ap_iou_thresholds, model, criterion, args)
    save_checkpoint(args, 'last', model, optimizer, scheduler, generator, virtual_optimizer, virtual_scheduler, save_cur=True)
...
def train_one_epoch(epoch, train_loader, DATASET_CONFIG, model, criterion, optimizer, scheduler, generator, virtual_optimizer, virtual_scheduler, config):
    stat_dict = {}  # collect statistics
    model.train()  # set model to training mode
    for batch_idx, batch_data_label_T in enumerate(train_loader):
        ...
        batch_data_label_S = generator(data_augmentation, scan_idx, use_height=False) # Now the batches are not parallel!!!
        ...
        loss, mmd_loss, _, end_points_T = criterion(end_points_S, end_points_T, DATASET_CONFIG,
                                     ...
                                     Is_mmd=True, epoch=epoch)

        optimizer.zero_grad()
        virtual_optimizer.zero_grad()
        (loss+0.000*mmd_loss).backward()
        ...
        '''
        # Forward pass
        end_points_S1 = model(inputs_S)
        end_points_S2 = model(detach_dict(inputs_S))
        end_points_T = model(inputs_T)

        # Compute loss and gradients, update parameters.
        for key in batch_data_label_S:
            assert(key not in end_points_S1)
            end_points_S1[key] = batch_data_label_S[key]
            assert(key not in end_points_S2)
            end_points_S2[key] = batch_data_label_S[key]
        for key in batch_data_label_T:
            assert(key not in end_points_T)
            end_points_T[key] = batch_data_label_T[key]

        temp1, mmd_loss, _, _ = criterion(end_points_S1, end_points_T, DATASET_CONFIG,
                                     num_decoder_layers=config.num_decoder_layers,
                                     query_points_generator_loss_coef=config.query_points_generator_loss_coef,
                                     obj_loss_coef=config.obj_loss_coef,
                                     box_loss_coef=config.box_loss_coef,
                                     ...
                                     heading_delta=config.heading_delta,
                                     Is_mmd=True, epoch=epoch)
        loss, temp2, _, end_points_T = criterion(end_points_S2, end_points_T, DATASET_CONFIG,
                                     num_decoder_layers=config.num_decoder_layers,
                                     query_points_generator_loss_coef=config.query_points_generator_loss_coef,
                                     obj_loss_coef=config.obj_loss_coef,
                                     box_loss_coef=config.box_loss_coef,
        ...
        optimizer.zero_grad()
        virtual_optimizer.zero_grad()
        (loss+mmd_loss+0.000*temp1+0.000*temp2).backward()
        ...
        if config.clip_norm > 0:
            grad_total_norm = 0.0
            grad_total_norm += torch.nn.utils.clip_grad_norm_(model.parameters(), config.clip_norm)
            grad_total_norm += torch.nn.utils.clip_grad_norm_(generator.parameters(), config.clip_norm)
        optimizer.step()
        scheduler.step()
        ...
        for key in end_points_T:
            if 'loss' in key or 'acc' in key or 'ratio' in key or 'max' in key:
...
if __name__ == '__main__':
    ...
    LOG_DIR = os.path.join('logs/log_{}/'.format(opt.dataset) + opt.log_dir, 'group_free',
