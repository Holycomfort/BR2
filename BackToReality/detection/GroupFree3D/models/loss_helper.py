...
def compute_objectness_loss_based_on_query_points(end_points, num_decoder_layers):
    ...
    for prefix in prefixes:
        ...
        if objectness_scores.shape[-1] == 2: objectness_scores = objectness_scores[:,:,1:] - objectness_scores[:,:,:1]
        criterion = SigmoidFocalClassificationLoss()
...
def compute_objectness_loss_based_on_query_points_weak(end_points, num_decoder_layers):
    ...
    for prefix in prefixes:
        ...
        end_points[f'{prefix}max_objectness_scores'] = nn.Sigmoid()(objectness_scores).max()
        end_points[f'{prefix}objectness_loss'] = objectness_loss
        objectness_loss_sum += objectness_loss
        ...
        # objectness_loss = torch.sum(objectness_loss * objectness_mask) / (torch.sum(objectness_mask)+1e-6) / B
        # end_points[f'{prefix}max_objectness_scores'] = nn.Softmax(dim=-1)(objectness_scores)[:,:,1].max()
        # end_points[f'{prefix}objectness_loss'] = objectness_loss
        # objectness_loss_sum += objectness_loss
...
def get_loss_virtual(end_points, config, num_decoder_layers,
             query_points_generator_loss_coef, obj_loss_coef, box_loss_coef, sem_cls_loss_coef,
             query_points_obj_topk=5,
             center_loss_type='smoothl1', center_delta=1.0,
             size_loss_type='smoothl1', size_delta=1.0,
    ...
    """
    if 'seeds_obj_cls_logits' in end_points.keys():
        query_points_generation_loss = compute_points_obj_cls_loss_hard_topk_weak(end_points, query_points_obj_topk)
...
def compute_iou_labels(end_points, pred_center, pred_sem_cls, pred_objectness, pred_heading_scores,
    ...
    dist1, object_assignment, _, _ = nn_distance(pred_center, center_label)
    euclidean_dist1 = torch.sqrt(dist1 + 1e-6)
...
def get_loss_DA(end_points_S, end_points_T, config, num_decoder_layers,
             ...
             size_loss_type='smoothl1', size_delta=1.0,
             heading_loss_type='smoothl1', heading_delta=1.0,
             Is_mmd=False, epoch=-1):
    """ Loss functions
    """
    loss = 0.5 * get_loss_virtual(end_points_S, config, num_decoder_layers, query_points_generator_loss_coef, obj_loss_coef, box_loss_coef, sem_cls_loss_coef,
    ...
    for prefix in prefixes:
        iou_labels, _, iou_assignment = compute_iou_labels(
            ...
            end_points_S, end_points_S[f'{prefix}center'], None, None, end_points_S[f'{prefix}heading_scores'],
            end_points_S[f'{prefix}heading_residuals'], end_points_S[f'{prefix}size_scores'],
            end_points_S[f'{prefix}size_residuals'], config_dict={'dataset_config': config})
        iou_pred = nn.Sigmoid()(end_points_S[f'{prefix}iou_scores'])
        # gt sem cls
        ...
        iou_loss_temp = huber_loss(iou_pred - iou_labels.detach(), delta=1.0)  # (B, K, 1)
        iou_loss_temp = iou_loss_temp.mean()
        end_points_S[f'{prefix}iou_loss'] = iou_loss_temp
        if epoch > 250:
            iou_loss += iou_loss_temp
        else:
            iou_loss += 0.000 * iou_loss_temp

    DA_loss = source_dloss + target_dloss
    loss += 10 * (DA_loss + iou_loss)
    if Is_mmd:
        mmd_dloss *= 10
...
def get_pseudo_labels(end_points, pred_center, pred_sem_cls, pred_size, pred_objectness, pred_heading_scores,
    ...
    # obj score threshold
    pred_objectness = nn.Sigmoid()(pred_objectness)
    # the second element is positive score
    pos_obj = pred_objectness[:, :, 0]
    neg_obj = 1 - pos_obj
    neg_objectness_mask = neg_obj > config_dict['obj_threshold']  # deprecated
    ...
    iou_pred = nn.Sigmoid()(end_points['last_iou_scores'])
    ...
    # print(cls_mask.sum(-1), dis_mask.sum(-1), objectness_mask.sum(-1), iou_mask.sum(-1))
    # sys.exit(0)
    ...
    pred_size_residuals = 0.85 * pred_size_residuals - 0.15 * mean_size_arr
    #######################
    '''
    ...
    center_label[(1 - label_mask).unsqueeze(-1).expand(-1, -1, 3).bool()] = -2000
...
def get_pseudo_detection_loss(end_points, config, config_dict, num_decoder_layers, box_loss_coef, sem_cls_loss_coef,
    ...
    seed_inds = end_points['seed_inds'].long()  # B,num_seed in [0,num_points-1]
    gt_center = end_points['unlabeled_center_label'][:, :, 0:3]  # B, K2, 3
    query_points_sample_inds = end_points['query_points_sample_inds'].long()
    aggregated_vote_xyz = end_points['query_points_xyz']
    batch_size = seed_inds.shape[0]
    K = query_points_sample_inds.shape[1]
    K2 = gt_center.shape[1]
    dist1, ind1, dist2, _ = nn_distance(aggregated_vote_xyz, gt_center) # dist1: BxK, dist2: BxK2
    euclidean_dist1 = torch.sqrt(dist1+1e-6)
    ...
    ## DA loss just for the stupid DDP
    # Target domain global
    target_dloss = end_points['global_d_pred'].mean()
    # S/T local & IoU Loss
    iou_loss = 0.0
    prefixes = ['last_']
    for prefix in prefixes:
        target_dloss += end_points[f'{prefix}local_d_pred'].mean()
        iou_loss += end_points[f'{prefix}iou_scores'].mean()
    # MMD
    mmd_dloss = end_points['shared_local_features'].mean()
    # IoU Loss
    SB_loss = target_dloss + mmd_dloss + iou_loss

    # means average proposal with prediction loss
    loss = 1.0 / (num_decoder_layers + 1) * (box_loss_coef * box_loss) + 0.000 * SB_loss
    loss *= 10
...
def get_loss_ST(end_points, ema_end_points, config, config_dict,
    ...
    pred_center = ema_end_points[f'{prefix}center'][labeled_num:]
    pred_sem_cls = ema_end_points[f'{prefix}sem_cls_scores'][labeled_num:]
    pred_size = ema_end_points[f'{prefix}pred_size'][labeled_num:]
    pred_objectness = ema_end_points[f'{prefix}objectness_scores'][labeled_num:]
    pred_heading_scores = ema_end_points[f'{prefix}heading_scores'][labeled_num:]
    pred_heading_residuals = ema_end_points[f'{prefix}heading_residuals'][labeled_num:]
    pred_size_scores = ema_end_points[f'{prefix}size_scores'][labeled_num:]
    pred_size_residuals = ema_end_points[f'{prefix}size_residuals'][labeled_num:]
