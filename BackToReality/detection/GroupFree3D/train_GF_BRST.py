...
from models import GroupFreeDetector_DA, get_loss_weak, get_loss_ST
...
def load_checkpoint(args, model, ema_model, optimizer, scheduler):
    logger.info("=> loading checkpoint '{}'".format(args.checkpoint_path_student))

    checkpoint = torch.load(args.checkpoint_path_student, map_location='cpu')
    #args.start_epoch = checkpoint['epoch'] + 1
    model.load_state_dict(checkpoint['model'], strict=False)
    #optimizer.load_state_dict(checkpoint['optimizer'])
    #scheduler.load_state_dict(checkpoint['scheduler'])
    logger.info("=> loaded successfully '{}' (epoch {})".format(args.checkpoint_path_student, checkpoint['epoch']))

    logger.info("=> loading checkpoint '{}'".format(args.checkpoint_path_teacher))
    checkpoint = torch.load(args.checkpoint_path_teacher, map_location='cpu')
    state_dict = checkpoint['model']
    new_state_dict = {}
    for k, v in state_dict.items():
        if "module" in k: new_name = k[7:]
        else: new_name = k
        new_state_dict[new_name] = v
    ema_model.load_state_dict(new_state_dict)
    logger.info("=> loaded successfully '{}' (epoch {})".format(args.checkpoint_path_teacher, checkpoint['epoch']))
...
def main(args):
    ...
    ema_model = get_model(args, DATASET_CONFIG, ema=True)
    ST_criterion = get_loss_ST
    weak_criterion = get_loss_weak
    if dist.get_rank() == 0:
    ...
    if args.checkpoint_path_student and args.checkpoint_path_teacher:
        assert os.path.isfile(args.checkpoint_path_student)
        assert os.path.isfile(args.checkpoint_path_teacher)
        load_checkpoint(args, model, ema_model, optimizer, scheduler)

    # Used for AP calculation
    CONFIG_DICT = {'dataset_config': DATASET_CONFIG, 'dataset': args.dataset,
                   'remove_empty_box': False, 'use_3d_nms': True, 'nms_iou': 0.25,
                   'use_old_type_nms': False, 'cls_nms': True, 'conf_thresh': 0.00,
                   'per_class_proposal': True, 'size_ratio': 0.2, 'obj_threshold': 0.3,
                   'iou_threshold': 0.25, 'use_lhs': True, 'samecls_match': False}
    ...
    for epoch in range(args.start_epoch, args.max_epoch + 1):
        ...
        train_one_epoch(epoch, train_loader, DATASET_CONFIG, CONFIG_DICT, model, ema_model, weak_criterion, ST_criterion, optimizer, scheduler, args)
        ...
        if epoch % args.val_freq == 0:
            evaluate_one_epoch(test_loader, DATASET_CONFIG, CONFIG_DICT, args.ap_iou_thresholds, model, args)
...
def train_one_epoch(epoch, train_loader, DATASET_CONFIG, CONFIG_DICT, model, ema_model, weak_criterion, ST_criterion, optimizer, scheduler, config):
    ...
    for batch_idx, batch_data_label in enumerate(train_loader):
        ...
        with torch.no_grad():
            ema_end_points = ema_model(inputs)

        # Compute loss and gradients, update parameters.
        for key in batch_data_label:
            assert (key not in end_points)
            end_points[key] = batch_data_label[key]
        weak_loss, end_points = weak_criterion(end_points, DATASET_CONFIG,
        ...
        ST_loss, end_points, _ = ST_criterion(end_points, ema_end_points, DATASET_CONFIG, CONFIG_DICT,
                                     num_decoder_layers=config.num_decoder_layers,
                                     box_loss_coef=config.box_loss_coef,
                                     sem_cls_loss_coef=config.sem_cls_loss_coef,
                                     center_loss_type=config.center_loss_type,
...
def evaluate_one_epoch(test_loader, DATASET_CONFIG, CONFIG_DICT, AP_IOU_THRESHOLDS, model, config):
    stat_dict = {}
