...
class ScannetDatasetConfig_md40(object):
    ...
    def size2class(self, size, type_name, require_grad=False):
        ''' Convert 3D box size (l,w,h) to size class and size residual '''
        size_class = self.type2class[type_name]
        size_residual = size - torch.Tensor(self.type_mean_size[type_name]) if require_grad else self.type_mean_size[type_name]
        if require_grad:
            return size_class, size_residual.detach(), size_residual / torch.Tensor(self.type_mean_size[type_name])
        else:
            return size_class, size_residual
